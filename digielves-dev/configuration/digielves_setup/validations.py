...
def validate_current_status(current_status):
    max_length = 20
    if len(current_status) > max_length:
        raise ValidationError(f"Current status should not exceed {max_length} characters.")
...
def is_valid_gym_name(gym_name):
    pattern = r'^[a-zA-Z0-9\s\-\',.()&]+$'
    if bool(re.match(pattern, gym_name)):
        raise ValidationError("Invalid Gym Name.")
    return True
