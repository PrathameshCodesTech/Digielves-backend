...
def generate_prescription(prescriptionData,  consultation_id):
    ...
    pdf.image('vibe.jpeg', x=10, y=13, w=45, h=22, type='', link='')
    ...
    patient_contact = data['consultation_for']['phone_no']
    print('zzzzzzzzzzzzzzzzzzzaaaaaaaaaaaaaaaaaaaaa')
    ...
    pdf.set_text_color(0,0,0)
    text = f"{note}"
    pdf.multi_cell(w=190, h=5, txt=str(text), fill= True, align='L')
    ...
    for row in tickets:
        ...
        for key in row.keys():
            ...
            if count in [1]:
                ...
                print("----------------------------")
                print(row['doses'])
                merged_text = str(row[key]) + " " + str(row['doses'])
